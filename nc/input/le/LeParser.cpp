...
// -----------------------------------------------------------------------------
#define EMIT(fields, name)                                                                                             \
    void fix_byte_order(name &h) { fields }
...
LeParser::LeParser() : core::input::Parser(QLatin1String("LE")) {}
...
void LeParser::doParse(QIODevice *in, core::image::Image *image, const LogToken &log) const {
    ...
    for (uint32_t oi = 0; oi < h.object_table_entries; ++oi) {
        ...
        auto section = std::make_unique<core::image::Section>(QString(QLatin1String(".seg%1")).arg(oi),
        ...
        if (!in->seek(off) || (bytes[oi] = in->read(len), bytes[oi].size() != len)) {
            throw ParseError(
                tr("Truncated object body at 0x%1:0x%2 for object %3").arg(off, 1, 16).arg(len, 1, 16).arg(oi));
        }
        ...
        image->addSymbol(std::make_unique<core::image::Symbol>(core::image::SymbolType::NOTYPE, s->name(),
                                                               ByteAddr(oh.relocation_base_address), s));
    }
    ...
    for (std::size_t npage = 0; npage < h.pages; ++npage) {
        ...
        ByteAddr page_virt_addr =
            image->sections()[seci]->addr() + (npage + 1 - sec_headers[seci].page_map_index) * h.memory_page_size;
        ...
        while (in->pos() < end) {
            ...
            if (fh.src == FIXUP_SRC_16BIT) {
                ...
                if (fh.flags == FIXUP_DST_FLAG_32BIT_TARGET_OFFSET) {
                    ...
                    throw ParseError(
                        tr("Fixup at 0x%1 has unsupported flags 0x%2").arg(start_mark, 1, 16).arg(fh.flags, 1, 16));
                }
            }
            ...
            image->addRelocation(
                std::make_unique<core::image::Relocation>(page_virt_addr + fh.srcoff,   // relocation virtual address
            ...
            if (width == 4) {
                ...
                bytes[seci].replace(page_virt_addr + fh.srcoff - image->sections()[seci]->addr(), // section offset
