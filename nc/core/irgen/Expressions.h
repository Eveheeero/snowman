...
public:
    explicit UnaryStatementBase(E expression) : mExpression(std::move(expression)) {}
...
public:
    BinaryStatementBase(L left, R right) : mLeft(std::move(left)), mRight(std::move(right)) {}
...
public:
    explicit KillStatement(E expression) : base_type(std::move(expression)) {}
};
...
public:
    JumpStatement(C condition, A address, ir::BasicBlock *thenBasicBlock, ir::BasicBlock *elseBasicBlock)
        : mCondition(std::move(condition)), mAddress(std::move(address)), mThenBasicBlock(thenBasicBlock),
          mElseBasicBlock(elseBasicBlock) {}
...
public:
    explicit CallStatement(E expression) : base_type(std::move(expression)) {}
};
...
public:
    ...
    explicit ExpressionBase(SmallBitSize size = 0) : mSize(size) {}

    SmallBitSize size() const { return mSize; }

    void setSize(SmallBitSize size) {
        assert(!mSize);
        mSize = size;
    }
...
public:
    explicit UnaryExpressionBase(E operand, SmallBitSize size = 0) : base_type(size), mOperand(std::move(operand)) {}
...
public:
    BinaryExpressionBase(L left, R right, SmallBitSize size = 0)
        : base_type(size), mLeft(std::move(left)), mRight(std::move(right)) {}
...
public:
    explicit ConstantExpression(ConstantValue value, SmallBitSize size = 0) : base_type(size), mValue(value) {}
...
public:
    explicit TermExpression(std::unique_ptr<ir::Term> term) : base_type(term->size()), mTerm(std::move(term)) {}

    TermExpression(TermExpression &&that) : base_type(std::move(that)), mTerm(std::move(that.mTerm)) {}

    TermExpression(const TermExpression &that) : base_type(that), mTerm(that.mTerm->clone()) {}
...
public:
    MemoryLocationExpression(const ir::MemoryLocation &memoryLocation)
        : base_type(memoryLocation.size()), mMemoryLocation(memoryLocation) {}

    MemoryLocationExpression(const MemoryLocationExpression &that)
        : base_type(that), mMemoryLocation(that.memoryLocation()) {}
...
struct binary_operator_kind<unsignedOperatorKind, ExpressionSignedness::UNSIGNED>
    : boost::mpl::int_<unsignedOperatorKind> {};

template <>
struct binary_operator_kind<ir::BinaryOperator::UNSIGNED_DIV, ExpressionSignedness::SIGNED>
    : boost::mpl::int_<ir::BinaryOperator::SIGNED_DIV> {};

template <>
struct binary_operator_kind<ir::BinaryOperator::UNSIGNED_REM, ExpressionSignedness::SIGNED>
    : boost::mpl::int_<ir::BinaryOperator::SIGNED_REM> {};

template <>
struct binary_operator_kind<ir::BinaryOperator::UNSIGNED_LESS, ExpressionSignedness::SIGNED>
    : boost::mpl::int_<ir::BinaryOperator::SIGNED_LESS> {};

template <>
struct binary_operator_kind<ir::BinaryOperator::UNSIGNED_LESS_OR_EQUAL, ExpressionSignedness::SIGNED>
    : boost::mpl::int_<ir::BinaryOperator::SIGNED_LESS_OR_EQUAL> {};

template <>
struct binary_operator_kind<ir::BinaryOperator::SHR, ExpressionSignedness::SIGNED>
    : boost::mpl::int_<ir::BinaryOperator::SAR> {};
...
struct binary_expression_operator_kind
    : public binary_operator_kind<unsignedOperatorKind, expression_signedness<E>::value> {};

// -------------------------------------------------------------------------- //
// Shortcuts
// -------------------------------------------------------------------------- //
inline IntrinsicExpression intrinsic(int kind = ir::Intrinsic::UNKNOWN) {
    return IntrinsicExpression(kind);
}

inline IntrinsicExpression undefined() {
    return intrinsic(ir::Intrinsic::UNDEFINED);
}

inline IntrinsicExpression return_address() {
    return intrinsic(ir::Intrinsic::RETURN_ADDRESS);
}

inline ConstantExpression constant(ConstantValue value, SmallBitSize size = 0) {
    return ConstantExpression(value, size);
}

inline TermExpression term(std::unique_ptr<ir::Term> term) {
    return TermExpression(std::move(term));
}

inline MemoryLocationExpression regizter(const arch::Register *reg) {
    return MemoryLocationExpression(reg->memoryLocation());
}

template <class E>
inline typename std::enable_if<IsExpression<E>::value, SignExpression<ExpressionSignedness::SIGNED, E>>::type
signed_(E expression) {
    return SignExpression<ExpressionSignedness::SIGNED, E>(std::move(expression));
}

template <class E>
inline typename std::enable_if<IsExpression<E>::value, SignExpression<ExpressionSignedness::UNSIGNED, E>>::type
unsigned_(E expression) {
    return SignExpression<ExpressionSignedness::UNSIGNED, E>(std::move(expression));
}

template <class E>
inline typename std::enable_if<IsExpression<E>::value, DereferenceExpression<E>>::type operator*(E expression) {
    return DereferenceExpression<E>(std::move(expression));
}

template <class E>
inline typename std::enable_if<IsExpression<E>::value, DereferenceExpression<E>>::type dereference(E expression,
                                                                                                   SmallBitSize size) {
...
template <class E>
inline typename std::enable_if<IsExpression<E>::value, UnaryExpression<ir::UnaryOperator::NOT, E>>::type
operator~(E expression) {
    return UnaryExpression<ir::UnaryOperator::NOT, E>(std::move(expression));
}

template <class E>
inline typename std::enable_if<IsExpression<E>::value, UnaryExpression<ir::UnaryOperator::NEGATION, E>>::type
operator-(E expression) {
    return UnaryExpression<ir::UnaryOperator::NEGATION, E>(std::move(expression));
}

template <class E>
inline typename std::enable_if<IsExpression<E>::value, UnaryExpression<ir::UnaryOperator::SIGN_EXTEND, E>>::type
sign_extend(E expression, BitSize size = 0) {
    return UnaryExpression<ir::UnaryOperator::SIGN_EXTEND, E>(std::move(expression), size);
}

template <class E>
inline typename std::enable_if<IsExpression<E>::value, UnaryExpression<ir::UnaryOperator::ZERO_EXTEND, E>>::type
zero_extend(E expression, BitSize size = 0) {
    return UnaryExpression<ir::UnaryOperator::ZERO_EXTEND, E>(std::move(expression), size);
}

template <class E>
inline typename std::enable_if<IsExpression<E>::value, UnaryExpression<ir::UnaryOperator::TRUNCATE, E>>::type
truncate(E expression, SmallBitSize size = 0) {
    return UnaryExpression<ir::UnaryOperator::TRUNCATE, E>(std::move(expression), size);
}

template <class L, class R>
inline typename std::enable_if<IsExpression<L>::value && IsExpression<R>::value,
                               BinaryExpression<ir::BinaryOperator::ADD, L, R>>::type
...
template <class L, class R>
inline typename std::enable_if<IsExpression<L>::value && IsExpression<R>::value,
                               BinaryExpression<ir::BinaryOperator::SUB, L, R>>::type
operator-(L left, R right) {
...
template <class L, class R>
inline typename std::enable_if<IsExpression<L>::value && IsExpression<R>::value,
                               BinaryExpression<ir::BinaryOperator::MUL, L, R>>::type
operator*(L left, R right) {
...
inline typename std::enable_if<
    ...
    BinaryExpression<binary_expression_operator_kind<ir::BinaryOperator::UNSIGNED_DIV, L>::value, L, R>>::type
operator/(L left, R right) {
...
inline typename std::enable_if<
    IsExpression<L>::value && IsExpression<R>::value,
    BinaryExpression<binary_expression_operator_kind<ir::BinaryOperator::UNSIGNED_REM, L>::value, L, R>>::type
operator%(L left, R right) {
...
template <class L, class R>
inline typename std::enable_if<IsExpression<L>::value && IsExpression<R>::value,
                               BinaryExpression<ir::BinaryOperator::AND, L, R>>::type
operator&(L left, R right) {
...
template <class L, class R>
inline typename std::enable_if<IsExpression<L>::value && IsExpression<R>::value,
                               BinaryExpression<ir::BinaryOperator::OR, L, R>>::type
operator|(L left, R right) {
...
template <class L, class R>
inline typename std::enable_if<IsExpression<L>::value && IsExpression<R>::value,
                               BinaryExpression<ir::BinaryOperator::XOR, L, R>>::type
operator^(L left, R right) {
...
template <class L, class R>
inline typename std::enable_if<IsExpression<L>::value && IsExpression<R>::value,
                               BinaryExpression<ir::BinaryOperator::SHL, L, R>>::type
operator<<(L left, R right) {
...
inline typename std::enable_if<
    IsExpression<L>::value && IsExpression<R>::value,
    BinaryExpression<binary_expression_operator_kind<ir::BinaryOperator::SHR, L>::value, L, R>>::type
operator>>(L left, R right) {
    return BinaryExpression<binary_expression_operator_kind<ir::BinaryOperator::SHR, L>::value, L, R>(std::move(left),
                                                                                                      std::move(right));
}

template <class L, class R>
inline typename std::enable_if<IsExpression<L>::value && IsExpression<R>::value,
                               BinaryExpression<ir::BinaryOperator::EQUAL, L, R>>::type
operator==(L left, R right) {
...
inline typename std::enable_if<
    IsExpression<L>::value && IsExpression<R>::value,
    BinaryExpression<binary_expression_operator_kind<ir::BinaryOperator::UNSIGNED_LESS, L>::value, L, R>>::type
operator<(L left, R right) {
...
inline typename std::enable_if<
    IsExpression<L>::value && IsExpression<R>::value,
    BinaryExpression<binary_expression_operator_kind<ir::BinaryOperator::UNSIGNED_LESS_OR_EQUAL, L>::value, L, R>>::type
operator<=(L left, R right) {
    return BinaryExpression<binary_expression_operator_kind<ir::BinaryOperator::UNSIGNED_LESS_OR_EQUAL, L>::value, L,
                            R>(std::move(left), std::move(right), 1);
}

template <class L, class R>
inline typename std::enable_if<
    IsExpression<L>::value && IsExpression<R>::value,
    BinaryExpression<binary_expression_operator_kind<ir::BinaryOperator::UNSIGNED_LESS, L>::value, R, L>>::type
operator>(L left, R right) {
...
inline typename std::enable_if<
    IsExpression<L>::value && IsExpression<R>::value,
    BinaryExpression<binary_expression_operator_kind<ir::BinaryOperator::UNSIGNED_LESS_OR_EQUAL, L>::value, R, L>>::type
operator>=(L left, R right) {
    return BinaryExpression<binary_expression_operator_kind<ir::BinaryOperator::UNSIGNED_LESS_OR_EQUAL, L>::value, R,
                            L>(std::move(right), std::move(left), 1);
}

template <class L, class R>
inline typename std::enable_if<IsExpression<L>::value && IsExpression<R>::value, AssignmentStatement<L, R>>::type
operator^=(L left, R right) {
    return AssignmentStatement<L, R>(std::move(left), std::move(right));
}

template <class E>
inline typename std::enable_if<IsExpression<E>::value, KillStatement<E>>::type kill(E expression) {
    return KillStatement<E>(std::move(expression));
}

template <class E>
inline typename std::enable_if<IsExpression<E>::value, JumpStatement<NullExpression, E>>::type jump(E thenAddress) {
    return JumpStatement<NullExpression, E>(NullExpression(), std::move(thenAddress), nullptr, nullptr);
}

inline JumpStatement<NullExpression, NullExpression> jump(ir::BasicBlock *targetBasicBlock) {
    return JumpStatement<NullExpression, NullExpression>(NullExpression(), NullExpression(), targetBasicBlock, nullptr);
}

template <class L, class R>
inline typename std::enable_if<IsExpression<L>::value && IsExpression<R>::value, JumpStatement<L, R>>::type
...
template <class E>
inline typename std::enable_if<IsExpression<E>::value, JumpStatement<E, NullExpression>>::type
...
template <class E>
inline typename std::enable_if<IsExpression<E>::value, CallStatement<E>>::type call(E target) {
    return CallStatement<E>(std::move(target));
}

inline HaltStatement halt() {
    return HaltStatement();
}

template <class L, class R>
inline typename std::enable_if<IsStatement<L>::value && IsStatement<R>::value, SequenceStatement<L, R>>::type operator,(
    L first, R second) {
...
public:
    explicit ExpressionFactory(const arch::Architecture *architecture) : mArchitecture(architecture) {}
    ...
    std::unique_ptr<ir::Term> createTerm(ExpressionBase<E> &expression) const {
        ...
        if (result && result->size() != expression.size()) {
            throw InvalidInstructionException(
                tr("Term %1 created from expression of size %2 has completely different size %3")
                    .arg(result->toString())
                    .arg(expression.size())
                    .arg(result->size()));
        }
...
protected:
     ...
     * \returns                        Newly created term for the given expression.
     */
    std::unique_ptr<ir::Term> doCreateTerm(TermExpression &expression) const { return std::move(expression.term()); }
    ...
    std::unique_ptr<ir::Term> doCreateTerm(DereferenceExpression<E> &expression) const {
        ...
        return std::make_unique<ir::Dereference>(createTerm(expression.operand()), ir::MemoryDomain::MEMORY,
                                                 expression.size());
    }
    ...
    std::unique_ptr<ir::Term> doCreateTerm(BinaryExpression<operatorKind, L, R> &expression) const {
        return std::make_unique<ir::BinaryOperator>(operatorKind, createTerm(expression.left()),
                                                    createTerm(expression.right()), expression.size());
    }
    ...
    std::unique_ptr<ir::Statement> doCreateStatement(AssignmentStatement<L, R> &statement) const {
        ...
        if (statement.left().size() != statement.right().size()) {
            throw core::irgen::InvalidInstructionException(tr("Cannot assign expressions of different sizes: %1 and %2")
                                                               .arg(statement.left().size())
                                                               .arg(statement.right().size()));
        }
...
public:
    ...
    ExpressionFactoryCallback(ExpressionFactory &factory, ir::BasicBlock *basicBlock,
        ...
        : mFactory(factory), mBasicBlock(basicBlock), mInstruction(instruction) {
