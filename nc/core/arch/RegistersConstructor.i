...
#undef REG
    ...
    for (unsigned i = 0; i < sizeof(registerDescriptions) / sizeof(RegisterDescription); i++) {
        registerRegister(new core::arch::Register(
            registerDescriptions[i].number, registerDescriptions[i].name,
            nc::core::ir::MemoryLocation(nc::core::ir::MemoryDomain::FIRST_REGISTER + registerDescriptions[i].domain,
                                         registerDescriptions[i].offset, registerDescriptions[i].size)));
    }
}
