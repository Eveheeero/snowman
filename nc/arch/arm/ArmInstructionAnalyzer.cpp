...
private:
    ...
    void createCondition(core::ir::BasicBlock *conditionBasicBlock, core::ir::BasicBlock *bodyBasicBlock,
                         core::ir::BasicBlock *directSuccessor) {
        using namespace core::irgen::expressions;
    ...
    void createBody(core::ir::BasicBlock *bodyBasicBlock) {
         ...
         */
        _[pc ^= constant(instruction_->addr() + 2 * instruction_->size())];
        ...
        case ARM_INS_ADD: {
            ...
            if (!handleWriteToPC(bodyBasicBlock)) {
                if (detail_->update_flags) {
                    _[n ^= signed_(operand(0)) < constant(0), z ^= operand(0) == constant(0), c ^= intrinsic(),
                      v ^= intrinsic(), less ^= ~(n == v), less_or_equal ^= less | z, below_or_equal ^= ~c | z];
                }
            }
            break;
        }
        case ARM_INS_AND: {
            ...
            if (!handleWriteToPC(bodyBasicBlock)) {
                if (detail_->update_flags) {
                    _[n ^= signed_(operand(0)) < constant(0), z ^= operand(0) == constant(0), c ^= intrinsic(),
                      less ^= ~(n == v), less_or_equal ^= less | z, below_or_equal ^= ~c | z];
                }
            }
            break;
        }
        ...
        case ARM_INS_CMN: {
            _[n ^= intrinsic(), c ^= unsigned_(operand(0)) < -operand(1), z ^= operand(0) == -operand(1),
              v ^= intrinsic(),
        ...
        case ARM_INS_LDM: {
            auto addr =
                MemoryLocationExpression(core::ir::MemoryLocation(core::ir::MemoryDomain::LAST_REGISTER, 0, 32));
        ...
        case ARM_INS_TST: {
            _[n ^= signed_(operand(0) & operand(1)) < constant(0), z ^= (operand(0) & operand(1)) == constant(0),
              c ^= intrinsic(), less ^= ~(n == v), less_or_equal ^= less | z, below_or_equal ^= ~c | z];
            break;
        }
        case ARM_INS_UXTAB: {
        ...
        for (int i = 0; i < memOperandIndex; ++i) {
            if (detail_->operands[i].type != ARM_OP_REG) {
                throw core::irgen::InvalidInstructionException(
                    tr("Expected the first %1 operand(s) to be register(s).").arg(memOperandIndex));
            }
        }
        if (detail_->operands[memOperandIndex].type != ARM_OP_MEM) {
            throw core::irgen::InvalidInstructionException(
                tr("Expected the operand #%1 to be a memory operand.").arg(memOperandIndex));
        }
    ...
    static std::unique_ptr<core::ir::Term> createTermForOperand(const cs_arm_op &operand, SmallBitSize sizeHint) {
        ...
        case ARM_OP_MEM:
            return std::make_unique<core::ir::Dereference>(createDereferenceAddress(operand),
                                                           core::ir::MemoryDomain::MEMORY, sizeHint);
        default:
            unreachable();
        }
    }
    ...
    static std::unique_ptr<core::ir::Term> createDereferenceAddress(const cs_arm_op &operand) {
        ...
        if (mem.index != ARM_REG_INVALID) {
            ...
            result = std::make_unique<core::ir::BinaryOperator>(
                mem.scale == 1 ? core::ir::BinaryOperator::ADD : core::ir::BinaryOperator::SUB, std::move(result),
                createRegisterAccess(mem.index), result->size());
        }

        if (mem.disp != 0) {
            result = std::make_unique<core::ir::BinaryOperator>(
                core::ir::BinaryOperator::ADD, std::move(result),
                std::make_unique<core::ir::Constant>(SizedValue(result->size(), mem.disp)), result->size());
        }

        return applyShift(operand, std::move(result));
    }

    static std::unique_ptr<core::ir::Term> applyShift(const cs_arm_op &operand,
                                                      std::unique_ptr<core::ir::Term> result) {
        ...
        case ARM_SFT_ASR_REG: {
            return std::make_unique<core::ir::BinaryOperator>(core::ir::BinaryOperator::SAR, std::move(result),
                                                              createShiftValue(operand), size);
        }
        case ARM_SFT_LSL: /* FALLTHROUGH */
        case ARM_SFT_LSL_REG: {
            return std::make_unique<core::ir::BinaryOperator>(core::ir::BinaryOperator::SHL, std::move(result),
                                                              createShiftValue(operand), size);
        }
        case ARM_SFT_LSR: /* FALLTHROUGH */
        case ARM_SFT_LSR_REG: {
            return std::make_unique<core::ir::BinaryOperator>(core::ir::BinaryOperator::SHR, std::move(result),
                                                              createShiftValue(operand), size);
        }
        ...
        case ARM_SFT_RRX_REG: {
            ...
            result = std::make_unique<core::ir::BinaryOperator>(
                ...
                std::make_unique<core::ir::UnaryOperator>(core::ir::UnaryOperator::ZERO_EXTEND, std::move(result),
                                                          size + 1),
                std::make_unique<core::ir::BinaryOperator>(
                    ...
                    std::make_unique<core::ir::UnaryOperator>(
                        ...
                        size + 1),
                    std::make_unique<core::ir::Constant>(SizedValue(sizeof(SmallBitSize), size + 1)), size + 1),
                size + 1);

            result = ror(std::move(result), createShiftValue(operand));

            return std::make_unique<core::ir::UnaryOperator>(core::ir::UnaryOperator::TRUNCATE, std::move(result),
                                                             size);
        }
        }
        unreachable();
    ...
    static std::unique_ptr<core::ir::Term> ror(std::unique_ptr<core::ir::Term> a, std::unique_ptr<core::ir::Term> b) {
        ...
        auto bb = std::make_unique<core::ir::BinaryOperator>(
            core::ir::BinaryOperator::SUB, std::make_unique<core::ir::Constant>(SizedValue(b->size(), size)),
            b->clone(), b->size());

        return std::make_unique<core::ir::BinaryOperator>(
            core::ir::BinaryOperator::OR,
            std::make_unique<core::ir::BinaryOperator>(core::ir::BinaryOperator::SHR, std::move(a), std::move(b), size),
            std::make_unique<core::ir::BinaryOperator>(core::ir::BinaryOperator::SHL, std::move(aa), std::move(bb),
                                                       size),
            size);
    }
    ...
    static std::unique_ptr<core::ir::Term> createShiftValue(const cs_arm_op &operand) {
        ...
        case ARM_SFT_RRX: {
            return std::make_unique<core::ir::Constant>(
                SizedValue(sizeof(operand.shift.value) * CHAR_BIT, operand.shift.value));
        }
...
#define REG(lowercase, uppercase)                                                                                      \
    case ARM_REG_##uppercase:                                                                                          \
        return ArmRegisters::lowercase();
            REG(apsr, APSR)
...
void ArmInstructionAnalyzer::doCreateStatements(const core::arch::Instruction *instruction,
