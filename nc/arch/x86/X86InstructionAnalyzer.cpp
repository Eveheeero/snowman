...
core::irgen::expressions::MemoryLocationExpression resizedRegister(const core::arch::Register *reg, SmallBitSize size) {
    return core::ir::MemoryLocation(reg->memoryLocation().domain(), 0, size);
}

core::irgen::expressions::MemoryLocationExpression temporary(SmallBitSize size) {
...
public:
    explicit X86InstructionAnalyzerImpl(const X86Architecture *architecture) : architecture_(architecture) {
    ...
    void createStatements(const X86Instruction *instr, core::ir::Program *program) {
        ...
        case UD_Iclc: {
            _[cf ^= constant(0), below_or_equal ^= cf | zf];
            break;
        }
        case UD_Icmc: {
            _[cf ^= ~cf, below_or_equal ^= cf | zf];
            break;
        }
        case UD_Istc: {
            _[cf ^= constant(1), below_or_equal ^= cf | zf];
            break;
        }
        case UD_Isahf: {
            ...
            _[cf ^= extractFlag(0), pf ^= extractFlag(1), af ^= extractFlag(3), zf ^= extractFlag(5),
              sf ^= extractFlag(6), less ^= ~(sf == of), less_or_equal ^= less | zf, below_or_equal ^= cf | zf];
            break;
        }
        case UD_Ifnstsw: {
            _[operand(0) ^= regizter(X86Registers::fpu_status_word())];
            break;
        }
        case UD_Ibswap: {
            if (operand(0).size() == 32) {
                _[operand(0) ^= ((unsigned_(operand(0)) >> constant(24))) |
                                ((unsigned_(operand(0)) >> constant(8)) & constant(0xFF00)) |
                                ((operand(0) << constant(8)) & constant(0xFF0000)) | ((operand(0) << constant(24)))];
            } else if (operand(0).size() == 64) {
                _[temporary(64) ^= ((operand(0) & constant(0xFFFFFFFFUL)) << constant(32)) |
        ...
        case UD_Iand: {
            ...
            _[cf ^= constant(0), pf ^= intrinsic(), zf ^= operand(0) == constant(0), sf ^= intrinsic(),
              of ^= constant(0), af ^= undefined(), less ^= ~(sf == of), less_or_equal ^= less | zf,
        ...
        case UD_Ibt: {
            _[cf ^= truncate(unsigned_(operand(0)) >> operand(1)), of ^= undefined(), sf ^= undefined(),
              zf ^= undefined(), af ^= undefined(), pf ^= undefined(), less ^= ~(sf == of), less_or_equal ^= less | zf,
              below_or_equal ^= cf | zf];
            break;
        }
        case UD_Icall: {
            ...
            _[regizter(sp) ^= regizter(sp) - constant(ip->size() / CHAR_BIT),
              *regizter(sp) ^= constant(instr->endAddr(), ip->size()), call(operand(0)),
              regizter(sp) ^= regizter(sp) + constant(ip->size() / CHAR_BIT)];
            break;
        }
        case UD_Icbw: {
            _[regizter(X86Registers::ax()) ^= sign_extend(regizter(X86Registers::al()))];
            break;
        }
        case UD_Icwde: {
            _[regizter(X86Registers::eax()) ^= sign_extend(regizter(X86Registers::ax()))];
            break;
        }
        case UD_Icdqe: {
            _[regizter(X86Registers::rax()) ^= sign_extend(regizter(X86Registers::eax()))];
            break;
        }
        case UD_Icqo: {
            _[regizter(X86Registers::rdx()) ^= signed_(regizter(X86Registers::rax())) >> constant(63)];
            break;
        }
        case UD_Icld: {
            _[df ^= constant(0)];
        ...
        case UD_Icmp: {
            _[cf ^= unsigned_(operand(0)) < operand(1), pf ^= intrinsic(), zf ^= operand(0) == operand(1),
              sf ^= signed_(operand(0)) < operand(1), of ^= intrinsic(), af ^= intrinsic(),

              less ^= signed_(operand(0)) < operand(1), less_or_equal ^= signed_(operand(0)) <= operand(1),
        ...
        case UD_Istosq: {
            ...
            _[increment ^= constant(accessSize / CHAR_BIT) -
                           constant(2 * accessSize / CHAR_BIT, si.memoryLocation().size()) * zero_extend(df)];
            ...
            case UD_Icmpsq: {
                ...
                body[cf ^= unsigned_(left) < right, pf ^= intrinsic(), zf ^= left == right, sf ^= signed_(left) < right,
                     of ^= intrinsic(), af ^= intrinsic(),

                     less ^= signed_(left) < right, less_or_equal ^= signed_(left) <= right,
            ...
            body[di ^= di + increment, si ^= si + increment];
        ...
        case UD_Icmpxchg: {
            ...
            _[cf ^= intrinsic(), pf ^= intrinsic(), zf ^= operand(0) == operand(1), sf ^= intrinsic(),
              of ^= intrinsic(), af ^= intrinsic(), less ^= ~(sf == of), less_or_equal ^= less | zf,
              below_or_equal ^= cf | zf,
            ...
            then[tmp ^= std::move(operand0), operand(0) ^= operand(1), operand(1) ^= tmp, jump(directSuccessor())];

            break;
        }
        case UD_Icpuid: {
            _[regizter(X86Registers::eax()) ^= intrinsic(), regizter(X86Registers::ebx()) ^= intrinsic(),
              regizter(X86Registers::ecx()) ^= intrinsic(), regizter(X86Registers::edx()) ^= intrinsic()];
            break;
        }
        case UD_Idec: {
            _[operand(0) ^= operand(0) - constant(1), pf ^= intrinsic(), zf ^= operand(0) == constant(0),
              sf ^= signed_(operand(0)) < constant(0), of ^= intrinsic(), af ^= intrinsic(), less ^= ~(sf == of),
              less_or_equal ^= less | zf, below_or_equal ^= cf | zf];
            break;
        }
        case UD_Idiv: {
            ...
            if (operand0.size() >= 16) {
                _[dx ^= unsigned_(ax) % std::move(operand0), ax ^= unsigned_(ax) / operand(0)];
            } else {
                _[dx ^= unsigned_(ax) % sign_extend(std::move(operand0)),
        ...
        case UD_Iidiv: {
            ...
            if (operand0.size() >= 16) {
                _[dx ^= signed_(ax) % std::move(operand0), ax ^= signed_(ax) / operand(0)];
            } else {
                _[dx ^= signed_(ax) % sign_extend(std::move(operand0)), ax ^= signed_(ax) / sign_extend(operand(0))];
            }

            _[cf ^= undefined(), of ^= undefined(), sf ^= undefined(), zf ^= undefined(), af ^= undefined(),
              pf ^= undefined()];
            break;
        }
        case UD_Iint: {
        ...
        case UD_Imul: {
            ...
            _[cf ^= intrinsic(), of ^= intrinsic(), sf ^= undefined(), zf ^= undefined(), af ^= undefined(),
              pf ^= undefined(), less ^= ~(sf == of), less_or_equal ^= less | zf, below_or_equal ^= cf | zf];
            break;
        }
        case UD_Iinc: {
            _[operand(0) ^= operand(0) + constant(1), pf ^= intrinsic(), zf ^= operand(0) == constant(0),
        ...
        case UD_Icmovs: {
            ...
            then[operand(0) ^= operand(1), jump(directSuccessor())];
        ...
        case UD_Ileave: {
            ...
            _[regizter(sp) ^= regizter(bp), regizter(bp) ^= *regizter(sp),
        ...
        case UD_Ineg: {
            _[cf ^= ~(operand(0) == constant(0)), operand(0) ^= -operand(0), pf ^= intrinsic(),
              zf ^= operand(0) == constant(0), sf ^= intrinsic(), of ^= intrinsic(), af ^= intrinsic(),
              less ^= ~(sf == of), less_or_equal ^= less | zf, below_or_equal ^= cf | zf];
            break;
        }
        case UD_Inop: {
            break;
        ...
        case UD_Ipop: {
            ...
            _[std::move(operand0) ^= *regizter(sp), regizter(sp) ^= regizter(sp) + constant(size / CHAR_BIT)];
            break;
        }
        case UD_Ipush: {
            auto sp = architecture_->stackPointer();
            ...
            if (op.type == UD_OP_IMM) {
                _[regizter(sp) ^= regizter(sp) - constant(sp->size() / CHAR_BIT),
                  *regizter(sp) ^= constant(getSignedValue(op, sp->size()), sp->size())];
            } else {
                auto operand0 = operand(0);
                auto size = operand0.size();

                _[regizter(sp) ^= regizter(sp) - constant(size / CHAR_BIT), *regizter(sp) ^= std::move(operand0)];
            }
            break;
        }
        case UD_Ipopfw: {
            auto sp = architecture_->stackPointer();
            _[*regizter(sp) ^= flags, regizter(sp) ^= regizter(sp) + constant(2)];
            break;
        }
        case UD_Ipopfd: {
            auto sp = architecture_->stackPointer();
            _[eflags ^= *regizter(sp), regizter(sp) ^= regizter(sp) + constant(4)];
            break;
        }
        case UD_Ipopfq: {
            auto sp = architecture_->stackPointer();
            _[rflags ^= *regizter(sp), regizter(sp) ^= regizter(sp) + constant(8)];
            break;
        }
        case UD_Ipushfw: {
            auto sp = architecture_->stackPointer();
            _[regizter(sp) ^= regizter(sp) - constant(2), *regizter(sp) ^= flags];
            break;
        }
        case UD_Ipushfd: {
            auto sp = architecture_->stackPointer();
            _[regizter(sp) ^= regizter(sp) - constant(4), *regizter(sp) ^= eflags & constant(0x00fcffff)];
            break;
        }
        case UD_Ipushfq: {
            auto sp = architecture_->stackPointer();
            _[regizter(sp) ^= regizter(sp) - constant(8), *regizter(sp) ^= rflags & constant(0x00fcffff)];
            break;
        }
        case UD_Iret: {
            auto sp = architecture_->stackPointer();
        ...
        case UD_Ishl: {
            ...
            _[cf ^= intrinsic(), sf ^= undefined(), zf ^= operand(0) == constant(0), pf ^= intrinsic(),
              less ^= ~(sf == of), less_or_equal ^= less | zf, below_or_equal ^= cf | zf];
            break;
        }
        case UD_Isar: {
        ...
        case UD_Isbb: {
            _[less ^= signed_(operand(0)) < operand(1) + zero_extend(cf),
              ...
              pf ^= intrinsic(), zf ^= operand(0) == constant(0) + zero_extend(cf),
              sf ^= signed_(operand(0)) < constant(0) + zero_extend(cf), of ^= intrinsic(), af ^= intrinsic()];
            break;
        }
        case UD_Iseta: {
        ...
        case UD_Isub: {
            _[less ^= signed_(operand(0)) < operand(1), less_or_equal ^= signed_(operand(0)) <= operand(1),
              cf ^= unsigned_(operand(0)) < operand(1), below_or_equal ^= unsigned_(operand(0)) <= operand(1),

              operand(0) ^= operand(0) - operand(1),

              pf ^= intrinsic(), zf ^= operand(0) == constant(0), sf ^= signed_(operand(0)) < constant(0),
              of ^= intrinsic(), af ^= intrinsic()];
            break;
        }
        case UD_Itest: {
            _[cf ^= constant(0), pf ^= intrinsic()];
            ...
            _[of ^= constant(0), af ^= undefined(), less ^= ~(sf == of), less_or_equal ^= less | zf,
              below_or_equal ^= cf | zf];
            break;
        }
        case UD_Ixchg: {
            ...
            _[tmp ^= operand(0), operand(0) ^= operand(1), operand(1) ^= tmp];
            break;
        }
        case UD_Ixor: {
            if (operandsAreTheSame(0, 1)) {
...
private:
    ...
    std::unique_ptr<core::ir::Term> createTermForOperand(std::size_t index) const {
        ...
        switch (operand.type) {
        case UD_NONE:
            throw core::irgen::InvalidInstructionException(
                tr("The instruction does not have an argument with index %1").arg(index));
        case UD_OP_MEM:
...
#define REG(ud_name, nc_name)                                                                                          \
    case UD_R_##ud_name:                                                                                               \
        return X86InstructionAnalyzer::createTerm(X86Registers::nc_name());
...
#undef REG_ST
    ...
    std::unique_ptr<core::ir::Dereference> createDereference(const ud_operand &operand) const {
        return std::make_unique<core::ir::Dereference>(createDereferenceAddress(operand),
                                                       core::ir::MemoryDomain::MEMORY, operand.size);
    }
        ...
        if (auto index = createRegisterAccess(operand.index)) {
            ...
            if (operand.scale > 1) {
                index = std::make_unique<core::ir::BinaryOperator>(
                    core::ir::BinaryOperator::MUL, std::move(index),
...
void X86InstructionAnalyzer::doCreateStatements(const core::arch::Instruction *instruction,
